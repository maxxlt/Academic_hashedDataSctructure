public class LQHashed_Chan {
    ...
    Candidate_Chan deleted;
    private Candidate_Chan[] data;

    public LQHashed_Chan(int length){
        int pct = (int)((1.0/loadingFactor - 1)*100.0);
        N = fourKPlus3(length,pct);
        data = new Candidate_Chan[N];
        deleted = new Candidate_Chan();
        for(int i = 0; i < N; i++)
            data[i] = null;
    }

    public boolean insert(Candidate_Chan newListing){
        boolean noError;
        boolean hit = false;
    ...
    public Candidate_Chan fetch(String targetKey){
        boolean noError;
        boolean hit = false;
        int pass, q, offset, ip;
    ...
    public boolean update(String targetKey, Candidate_Chan newListing){
